...
action_size = 10  # Number of discrete actions per dimension
action_values = np.linspace(action_space.low, action_space.high, action_size)
...
for e in range(args.num_episodes):
    ...
    for time in range(500) :
        ...
        translated_action = np.reshape( np.array( [ action_values[action] ] ) , (1,) )
        next_state , reward , done , truncated , info = env.step(translated_action)
